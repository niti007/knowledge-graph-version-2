...
from app.llm.tiering import Task
...
def build_agent_graph(toolbox: Toolbox, settings: Settings | None = None,
    ...
    loop runs deterministically with no provider call.
    """
    ...
    def agent_node(state: AgentState) -> dict:
        ...
        if iterations >= limit:
            ...
            reply = base.invoke(messages + [HumanMessage(content=FINAL_TURN_NUDGE)])
        ...
        for call in getattr(last, "tool_calls", []) or []:
            ...
            if tool is None:
                ...
                try:
                    ...
                    # The tool schema rejected the arguments before the tool ran
                    # -- graph_query's `template` is a Literal, so an injected
                    # Cypher string never reaches Neo4j. Tell the model what the
                    # legal values are so it can correct itself in one round.
...
def run_agent(question: str, toolbox: Toolbox | None = None,
    ...
    try:
        app = build_agent_graph(tb, settings=s, model=model)
        ...
        final = app.invoke(
            {"messages": messages, "iterations": 0,
             "max_iterations": s.agent_max_iterations},
            # One super-step per node visit; the real bound is the unbinding in
            # agent_node. This only stops a pathological graph from spinning.
